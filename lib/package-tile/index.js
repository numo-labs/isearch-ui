...
export default class PackageTile extends Component {
  ...
  formatDiscountPrice () {
    ...
    return (discountPrice && discountPrice !== '0') ? (totalPriceBeforeDiscount + ',-') : '';
  }
  render () {
    const { packageOffer, viewHotel, setHotelPage, itemId, bucketId } = this.props;
    ...
    const primaryImgSrc = constructUrl(hotelImage);
    return (
      <Link to={`/hotel/${bucketId}/${itemId}`}>
        <div
          onClick={() => {
            viewHotel();
          ...
          <div className='descriptionContainer'>
            ...
            <div className='bottomContainer'>
              ...
              <div className='descriptionRow'>
                ...
                <div className='price'>{`${packageOffer.price.total},-`}</div>
              </div>
            </div>
          </div>
        </div>
