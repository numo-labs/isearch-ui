...
describe('<SearchSummary /> Component', () => {
  ...
  it('When the visibleTab state is set to edit-details the EditDetails component should be the only child', (done) => {
    wrapper.setState({visibleTab: 'edit-details'});
    const children = wrapper.children().nodes;
    const firstChild = children[0].type;
    const editDetails = wrapper.find('EditDetails').node.type;
    expect(children).to.have.length(1);
    expect(firstChild).to.deep.equal(editDetails);
    done();
  });
  it('When the visibleTab state is set to tags the TagView component should be the only child', (done) => {
    wrapper.setState({visibleTab: 'tags'});
    const children = wrapper.children().nodes;
    const firstChild = children[0].type;
    const tagView = wrapper.find('TagView').node.type;
    expect(children).to.have.length(1);
    expect(firstChild).to.deep.equal(tagView);
    done();
  });
});
