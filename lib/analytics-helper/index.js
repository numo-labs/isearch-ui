import defaultTag from '../../src/constants/default-tag';
...
const _getItemId = (item) => {
  if (_itemIdMapping[item.type]) {
    return _itemIdMapping[item.type](item);
  } else {
    return item.id;
  }
};
...
export const analyticsAddTagObject = (displayName, currentTags, context) => {
  const initialTagIndex = currentTags.indexOf(defaultTag.displayName);
...
/**
 * Creates analytics object for dismissed tiles
 * @param itemId
 * @returns {{event: string, tile: string}}
 */
export const analyticsRemoveTile = (itemId) => {
  return {
    event: 'dismissTile',
    tileId: itemId
  };
